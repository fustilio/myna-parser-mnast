...
export namespace Myna {
    ...
    export class Rule
    {
        ...
        parser: ((p: ParseState) => boolean) | null = null;
    ...
    export function tokenize(r:Rule, s:string) : AstNode[]
    {
        ...
        const result = Myna.parse(r.ast.zeroOrMore, s);
    ...
    export function parse(r: Rule, s: string): AstNode {
        ...
        if (!(r instanceof AstRule))
            throw new ParserError('Rule must be an instance of AstRule');
        if (!r.parser(p)) 
            return null;
